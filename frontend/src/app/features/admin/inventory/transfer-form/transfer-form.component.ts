  ...
  template: `
    <div class="p-6 max-w-2xl mx-auto">
      ...
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Transfer Inventory</h1>
      </div>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate class="space-y-5">
        ...
        <!-- Source Location -->
        <div>
          <label for="source_location_id" class="block text-sm font-medium text-gray-700 mb-1">
          ...
          @if (loadingLocations()) {
            ...
            <select
              ...
              formControlName="source_location_id"
              class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
              [class.border-red-400]="isInvalid('source_location_id')"
              [class.border-gray-300]="!isInvalid('source_location_id')"
            >
          ...
          @if (isInvalid('source_location_id')) {
            <p class="mt-1 text-xs text-red-600" role="alert">Source location is required.</p>
          }
        </div>
        ...
        <div>
          ...
          @if (loadingLocations()) {
            ...
            <select
              ...
              class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
              [class.border-red-400]="isInvalid('destination_location_id')"
              [class.border-gray-300]="!isInvalid('destination_location_id')"
            >
          ...
          @if (isInvalid('destination_location_id')) {
            <p class="mt-1 text-xs text-red-600" role="alert">Destination location is required.</p>
          }
        </div>
        ...
        @if (submitSuccess()) {
          <div
            ...
            role="status"
          >
            Inventory transferred successfully. Redirecting…
          </div>
        }
        ...
        <div class="flex items-center gap-3 pt-2">
          <button
            ...
            class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white text-sm font-semibold rounded-lg transition focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
          >
...
export class TransferFormComponent implements OnInit {
  ...
  readonly form = this.fb.group({
    ...
    source_location_id: ['', [Validators.required]],
  ...
  onSubmit(): void {
    ...
    this.inventoryService.transfer({
      ...
      source_location_id: Number(raw.source_location_id),
      destination_location_id: Number(raw.destination_location_id),
